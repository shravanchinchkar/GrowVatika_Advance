...
export const ExploreCard: React.FC<ExploreCardProps> = ({ cardData }) => {
  ...
  return (
    <div
      key={cardData.id}
      className="group relative flex new-sm:w-[10.68413rem] new-sm:h-[9.09rem] lg:w-[17rem] lg:h-[13em] xl:w-[19.6875rem] xl:h-[16.75rem] rounded-[28px] overflow-hidden shadow-explore-custom cursor-pointer"
    >
      ...
      <div className="absolute inset-0 grid grid-cols-2 transform transition-transform duration-300 group-hover:-translate-y-full">
        ...
        <div className="bg-white new-sm:px-[0.7rem] new-sm:py-[0.5rem] new-sm-1:p-[0.6rem] lg:p-[0.5rem] xl:p-[1rem] flex flex-col justify-between">
          <div className="uppercase new-sm:text-[0.7rem] new-sm-1:text-[16px] font-medium">
            {cardData.title}
          </div>
          <div>
      ...
      <div className="transform transition-transform duration-300 translate-y-full group-hover:translate-y-0 font-[Poppins] bg-[#123524B8] w-[100%] flex flex-col items-start lg:gap-0 xl:gap-[0.2rem]">
        ...
        <div className="text-[#CBD0D3] new-sm:text-[0.8rem] new-sm-1:text-[1rem] font-normal w-[100%] flex flex-col">
          {cardData.onHover.map((item: string) => {
            return (
              <div
                key={uuidv4()}
                className="w-[100%] new-sm:h-[25px] new-sm-1:h-[40px] lg:h-[40px] xl:h-[40px] flex items-center new-sm:pl-[1.2rem] new-sm-1:pl-[2rem] hover:bg-[#123524] hover:drop-shadow-lg"
              >
                {item}
              </div>
            );
