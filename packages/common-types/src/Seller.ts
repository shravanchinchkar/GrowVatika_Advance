...
export const SellerDataSchema = zod
  .object({
    // Add profile picture field - required
    profilePicture: zod.instanceof(File).optional(), // Optional
  ...
  .refine(
    (data) => {
      // Custom validation: require either profilePicture OR profilePictureURL
      return data.profilePicture || data.profilePictureURL;
    },
    {
      message: "Profile picture is required",
      path: ["profilePicture"], // Show error on profilePicture field
    }
  );
